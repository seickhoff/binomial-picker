...
export function plinkPeg(index: number): void {
  ...
  // Two partials an octave apart, the upper one shorter: a small hard object
  // struck, rather than a tuning fork. Detuned a touch per strike so the same
  // peg twice running isn't the identical sound.
  const detune = 1 + (Math.random() - 0.5) * 0.01
  const body = context.createGain()
  body.gain.setValueAtTime(VOICE_GAIN, now)
  body.gain.exponentialRampToValueAtTime(0.0001, now + length)
  ...
  for (const [multiple, share, decay] of [
    [1, 1, 1],
    [2, 0.4, 0.55],
  ] as const) {
    const tone = context.createOscillator()
    tone.type = 'sine'
    tone.frequency.value = frequency * multiple * detune
  ...
  voices += 1
  partials[partials.length - 1].onended = () => {
    voices -= 1
    place.disconnect()
  }
}
